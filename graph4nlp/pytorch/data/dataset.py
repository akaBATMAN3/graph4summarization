...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # inference
        self.for_inference = for_inference #false
        # Processing-specific attributes
        self.tokenizer = tokenizer # 使用nltk推荐的word tokenizer
        self.lower_case = lower_case
        self.pretrained_word_emb_name = pretrained_word_emb_name # glove 840B 300d
        ...
        self.max_word_vocab_size = max_word_vocab_size # 70,000
        self.min_word_vocab_freq = min_word_vocab_freq # 1

        # self.pretrained_word_emb_file = pretrained_word_emb_file
        self.topology_builder = topology_builder # DependencyBasedGraphConstruction
        self.topology_subdir = topology_subdir
        self.use_val_for_vocab = use_val_for_vocab #False
    ...
    def processed_dir(self) -> str:
        return os.path.join(self.root, "processed", self.topology_subdir) # processed/denpendency
    ...
    def read_raw_data(self):
        ...
        if self.for_inference:
            ...
            return
        #print(self.raw_file_paths["train"])
        self.train = self.parse_file(self.raw_file_paths["train"]) # 读取raw/train_1w.json得到包含Text2TextDataItem实例的list
        self.test = self.parse_file(self.raw_file_paths["test"]) # 读取raw/test.json
        if "val" in self.raw_file_paths.keys():
            self.val = self.parse_file(self.raw_file_paths["val"]) #读取raw/val.json
    ...
    def _build_topology_process(
        ...
        if static_or_dynamic == "static":
            ...
            if topology_builder == IEBasedGraphConstruction:
                props_coref = {
                    "annotators": "tokenize, ssplit, pos, lemma, ner, parse, coref", # 分词，分句，词性标注，词形还原，命名实体识别，语义解析，共指解析
                    "tokenize.options": "splitHyphenated=true,normalizeParentheses=true," # 分隔连字符
                    "normalizeOtherBrackets=true",
                    "tokenize.whitespace": False, # 并非遇到空格就分词
                    "ssplit.isOneSentence": False,
                    "outputFormat": "json",
                }
                props_openie = {
                    "annotators": "tokenize, ssplit, pos, ner, parse, openie", # 开放信息抽取
                    "tokenize.options": "splitHyphenated=true,normalizeParentheses=true,"
                    "normalizeOtherBrackets=true",
                    "tokenize.whitespace": False,
                    "ssplit.isOneSentence": False,
                    "outputFormat": "json",
                    "openie.triple.strict": "true", # 严格抽取
                }
                ...
                processor_args = {
                    ...
                    "tokenize.whitespace": True, # 按空格分词
            ...
            pop_idxs = []
            for cnt, item in enumerate(data_items): # 对每个data_item in Text2TextDataItem处理
                if cnt % 1000 == 0:
                    print("Port {}, processing: {} / {}".format(port, cnt, len(data_items)))
                try:
                    #根据不同建图方法为每篇article建立不同图结构
                    graph = topology_builder.static_topology(
                        raw_text_data=item.input_text,
                        nlp_processor=processor,
                        processor_args=processor_args,
                        merge_strategy=merge_strategy, # ie:null
                        edge_strategy=edge_strategy, # ie:as_node
                        verbose=False,
                    )
                    item.graph = graph # 新属性
                except Exception as msg:
                    pop_idxs.append(cnt)
                    item.graph = None
                    warnings.warn(RuntimeWarning(msg))
                ret.append(item)
            ret = [x for idx, x in enumerate(ret) if idx not in pop_idxs] # 重新收集每篇article的data item
    ...
    def build_vocab(self):
        ...
        vocab_model = VocabModel.build(
            ...
            data_set=data_for_vocab, # train
            tokenizer=self.tokenizer, # nltk
            lower_case=self.lower_case, # true
            ...
            pretrained_word_emb_name=self.pretrained_word_emb_name, # glove
            ...
            word_emb_size=self.word_emb_size, # 300
        )
    ...
    def _process(self):
        ...
        os.makedirs(self.processed_dir, exist_ok=True) # processed/dependency
...
class Text2TextDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(Text2TextDataset, self).__init__(
            ...
            topology_builder=topology_builder, # DependencyBasedGraphConstruction
    ...
    def _vectorize_one_dataitem(cls, data_item, vocab_model, use_ie=False):
        ...
        for node_idx in range(graph.get_node_num()):
            node_token = graph.node_attributes[node_idx]["token"] # 获取结点所代表单词
            node_token_id = vocab_model.in_word_vocab.getIndex(node_token, use_ie) # 猜测是根据单词获取其在vocab中的索引
        ...
        if use_ie:
            ...
            token_matrix = torch.tensor(token_matrix, dtype=torch.long) # 大小为结点数*1
            graph.node_features["token_id"] = token_matrix

        if use_ie and "token" in graph.edge_attributes[0].keys(): # false
            edge_token_matrix = []
        ...
        if isinstance(tgt, str):
            tgt_token_id = vocab_model.out_word_vocab.to_index_sequence(tgt) # output_text中的单词转换为单词索引
            tgt_token_id.append(vocab_model.out_word_vocab.EOS) # 加上终止符
