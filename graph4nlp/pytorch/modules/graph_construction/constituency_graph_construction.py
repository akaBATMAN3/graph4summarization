...
class ConstituencyBasedGraphConstruction(StaticGraphConstructionBase):
    ...
    def static_topology(
        ...
        prune : int, option=[0,1,2]
            ...
            ``0``: No pruning.
            ``1``: Prune pos nodes.
            ``2``: Prune nodes with both in-degree and out-degree of 1. # 修剪掉单个有自环的结点
        ...
        for index in range(len(parsed_output)): # 对分句后的每句句子作处理
            #print(len(parsed_output))
            #print(parsed_output[index]['parse'])
            #print(parsed_output)
            #print(parsed_output[index])
            output_graph_list.append(
                cls._construct_static_graph(
                    parsed_output[index], # 每句句子的pos，依赖，tokens信息传入
                    index,
                    sequential_link=seq_link, # false
                    bisequential_link=biseq_link, # true
                    top_down=top_down, # false
                    add_pos_node=add_pos_nodes, # false
                    cut_line_node=cut_line_node, # true
                )
            )
        #print(len(output_graph_list))
    ...
    def _construct_static_graph(
        ...
        parsed_sentence_data = parsed_object["parse"] # pos
        for punc in [u"(", u")"]:
            #print(punc)
            parsed_sentence_data = parsed_sentence_data.replace(punc, " " + punc + " ")
            #print(parsed_sentence_data)
        ...
        #print(parse_list)
        # 去掉重复括号，去掉root，倒数第二个.换成period，存储在list中
        res_graph = GraphData()
        pstack = Stack()
        idx = 0
        cnt_word_node = -1
        while idx < len(parse_list):
            if parse_list[idx] == "(":
                ...
                #print(res_graph.get_node_num())
                res_graph.node_attributes[res_graph.get_node_num() - 1] = {
                    "token": parse_list[idx + 1], # 左括号后紧跟的词性作为token
                    "type": 1,
                    "position_id": None,
                ...
                pstack.push(res_graph.get_node_num() - 1) # 单词词性入栈
                if pstack.size() > 1:
                    node_2 = pstack.pop()
                    node_1 = pstack.pop()
                    if top_down:
                        ...
                        res_graph.add_edge(node_2, node_1) # 为单词词性添加边，右边的连向左边的
                    pstack.push(node_1)
                    pstack.push(node_2)
            elif parse_list[idx] == ")":
                pstack.pop() # 单词词性出栈
            elif parse_list[idx + 1] == u")" and parse_list[idx] != u")": # 遇到单词
                cnt_word_node += 1
                if add_pos_node:
                    res_graph.add_nodes(1)
                else:
                    assert res_graph.node_attributes[res_graph.get_node_num() - 1]["type"] != 0 # 确认当前结点为词性结点
                #print(res_graph.get_node_num() - 1)
                res_graph.node_attributes[res_graph.get_node_num() - 1] = {
                    "token": parse_list[idx], # 遇到单词替换掉和单词在同一括号内所对应的单词词性
                    "type": 0,
        ...
        for n in res_graph.node_attributes:
            if n["type"] == 0 and n["position_id"] > max_pos:
                max_pos = n["position_id"] # 获取最大单词数量

        min_pos = 99999
        for n in res_graph.node_attributes:
            if n["type"] == 0 and n["position_id"] < min_pos:
                min_pos = n["position_id"] # 最小单词数量
    ...
    def _graph_connect(
        ...
        if _len_graph_ > 1:
            ...
            for index in range(_len_graph_):
                ...
                merged_graph.node_attributes.extend(graph_i_nodes_attributes) # 获取所有结点属性

            _cnt_edge_num = 0
            for g_idx in range(_len_graph_): # 将所有子图的边进行连接
                tmp_edges = graph_list[g_idx].get_all_edges() # 当前子图的所有边
                current_node_num = graph_list[g_idx].get_node_num() # 当前子图结点数
                for _edge in tmp_edges:
                    merged_graph.add_edge(_edge[0] + _cnt_edge_num, _edge[1] + _cnt_edge_num)
                _cnt_edge_num += current_node_num

            for index in range(_len_graph_ - 1): # 为子图也就是句子间添加双向边
                # get head node
                head_node = -1
        ...
        if reformalize:
            ...
            for i_0, i_1 in enumerate(new_list_for_nodes):
                if i_1["type"] == 0:
                    list_for_word_nodes.append((i_0, i_1)) # word
                else:
            ...
            for item in list_for_word_nodes:
                merged_graph.node_attributes.append(item[1]) # word node先入
    ...
    def _cut_line_node(cls, input_graph: GraphData):
        ...
        for idx, _ in enumerate(input_graph.node_attributes):
            edge_arr = input_graph.get_all_edges() # list[(src, tgt)]边的组合组成的list
            cnt_in = 0
            cnt_out = 0
            for e in edge_arr:
                if idx == e[0]:
                ...
                if idx == e[1]:
                    cnt_in += 1
                    in_ = e[0]
            if cnt_in == 1 and cnt_out == 1: # 删除线性排列的结点
        ...
        for idx, n in enumerate(input_graph.node_attributes):
            if idx not in idx_to_be_deleted: # 不需删除的结点正常建图
                res_graph.add_nodes(1)
