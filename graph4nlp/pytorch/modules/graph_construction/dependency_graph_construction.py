...
class DependencyBasedGraphConstruction(StaticGraphConstructionBase):
    ...
    def static_topology(
       ...
       sequential_link =True,
        ...
        sub_graphs = []
        for parsed_sent in parsed_results: # 直接将整个article看作一句句子
        ...
        joint_graph = cls._graph_connect(sub_graphs, merge_strategy) # 不必合并，一篇article就是一张图
        return joint_graph
    ...
    def _construct_static_graph(cls, parsed_object, edge_strategy=None, sequential_link=True):
        ...
        node_objects = parsed_object["node_content"] # token信息
        ...
        for dep_info in parsed_object["graph_content"]: # 依赖信息
            if edge_strategy is None or edge_strategy == "homogeneous":
                ret_graph.add_edge(dep_info["src"], dep_info["tgt"]) # 直接根据依赖关系连接各token
        ...
        if sequential_link and len(sequential_list) > 1: # false
            for st, ed in zip(sequential_list[:-1], sequential_list[1:]):
                try:
                    ret_graph.edge_ids(st, ed)
                    #print(ret_graph.edge_ids(st, ed))
                except KeyError:
