...
def prepare_ext_vocab(batch_graph, vocab, gt_str=None, device=None):
    ...
    """
    oov_dict = copy.deepcopy(vocab.in_word_vocab) # 使用源单词建立oov_dict
    ...
    unk_token = [batch_graph.node_attributes[index]["token"] for index in unk_index] # 获取所有unk对应单词
    oov_dict._add_words(unk_token) # 将oov单词添加进oov_dict
