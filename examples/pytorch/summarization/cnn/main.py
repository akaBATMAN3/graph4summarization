...
#22.3.17 change graph_construction to graph_embedding_initialization
...
class SumModel(nn.Module):
    ...
    def forward(self, data, oov_dict=None, require_loss=True):
        ...
        if require_loss:
            ...
            tgt = tgt[:, :min_length]
            loss = self.loss_calc( # 利用真值和预测概率计算损失
                prob,
                label=tgt,
    ...
    def load_checkpoint(cls, model_path):
        ...
        #return torch.load(model_path, map_location=torch.device("cpu")) # for inference
...
class ModelHandler:
    ...
    def _build_dataloader(self):
        ...
        dataset.train = dataset.train[: self.config["n_samples"]] # 选取样本长度
    ...
    def train(self):
        ...
        for epoch in range(self.config["epochs"]):
            ...
            for i, data in enumerate(self.train_dataloader):
                ...
                oov_dict = None
                if self.use_copy:
                    oov_dict, tgt = prepare_ext_vocab( # 利用复制机制构建oov_dict和tgt_seq，将原始vocab扩展到包含oov的vocab
                        data["graph_data"],
                        self.vocab,
                ...
                logits, loss = self.model(data, oov_dict=oov_dict, require_loss=True) # 预测概率和损失
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(dataloader):
                ...
                if self.use_copy:
                    ...
                    oov_dict = None
                    ref_dict = self.vocab.out_word_vocab
                    #22.3.17 change graph_topology to graph_initializer
